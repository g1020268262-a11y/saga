...
As an ideal authorization objective motivating this study, we call the
end-to-end requirement \emph{policy--authorization consistency}:
every accepted access must be traceable to an applicable authorization decision.
The ideal requirement ranges over every execution and accepted-access event $e$:
\[
...
\]
In addition to this end-to-end property, we consider authorization-soundness
conditions at intermediate protocol boundaries, including Provider grants
and credential issuance. These intermediate events are not themselves
instances of $\mathsf{AccessAccepted}$.
...
where applicable, operation, and remain applicable under the relevant policy
state. Where a credential mediates access, the decision must also be linked to the grant and
...
This requirement motivates the analysis; it is not a theorem established by
this paper. We do not attempt to prove the complete $\mathsf{Justifies}$
relation or the authorization correctness of SAGA as a whole. We study a
specific class of authorization decision inconsistencies: disagreement
between a specification-side decision and an implementation-observed decision
for the recorded policy input and matched identity. The subsequent protocol
analysis concerns conditional consequences of that divergence under explicit
symbolic model assumptions, not authorization correctness in deployed executions.
...
For the symbolic analysis, we fix the rulebook throughout an execution and
consider either no matching rule or a unique highest-scoring match. Equal-score conflicts require
...
Under this static-policy scope, the intermediate condition at the Provider
boundary is
\[
  \mathsf{ProviderGrant}(I,R)
  \Longrightarrow \mathsf{Allow}_{\mathrm{spec}}(P_R,I),
\]
whereas a necessary permission condition for end-to-end consistency at the
final message-acceptance boundary is
\[
  \mathsf{MessageAccepted}(R,I,T,m)
  \Longrightarrow \mathsf{Allow}_{\mathrm{spec}}(P_R,I).
\]
The intermediate condition at token issuance is
\[
  \mathsf{TokenIssued}(R,I,T)
  \Longrightarrow \mathsf{Allow}_{\mathrm{spec}}(P_R,I).
\]
All three implications range over every occurrence of the respective events
and all their arguments. The intermediate conditions are separate obligations,
not logical consequences of end-to-end consistency alone. These implications
express permission at their respective boundaries; they do not replace the
decision-to-access linkage required by
...
Chapter~6 formalizes selected consequences of decision divergence under stated
assumptions; it does not establish the complete justification relation above.

\paragraph{Research question and scope.}
How can authorization decision inconsistencies be identified from recorded
implementation evidence and specification-side evaluation, and their protocol
consequences analyzed under explicit symbolic model assumptions?
For the recorded SAGA case, we compare an independently evaluated specification
decision with the archived implementation-observed decision, check their
evidence provenance, and reconstruct a divergence scenario for symbolic
consequence analysis. This separates decision comparison, evidence-to-scenario
transformation, and verification of selected model properties. Matching the
recorded inputs and tracking their provenance does not establish that the
observation and symbolic witness belong to the same real execution.
...
The following threat scenario motivates the symbolic analysis. Its identity,
policy, and trust assumptions are not observations of a deployed service run.
...
For the divergence scenario, we assume a common fixed rulebook for the Provider
and receiving agent. In the symbolic model, their enforcement points consume
the supplied implementation-side decision; they do not execute that rulebook
or the Python matcher. The function-level observation does not establish that
both deployed consumers used the same policy or consumed the recorded decision.
Cryptographic checks are assumed effective. We analyze possible consequences
of a decision inconsistent with the intended policy, without compromise of
either enforcement point. The prescribed initiating-peer process represents
the modeled interaction, not arbitrary adversary-controlled application behavior.
...
The results in this paper concern reachability within the specified symbolic
scenario. They do not demonstrate a deployed attack or a real service execution
connecting the matcher observation to message acceptance.
